...
const registerBuyerSchema = Joi.object({
  ...
  company_latitude: Joi.number().min(-90).max(90).allow(null, '').optional(),
  company_longitude: Joi.number().min(-180).max(180).allow(null, '').optional(),
  address: Joi.string().allow('').max(500),
  latitude: Joi.number().min(-90).max(90).allow(null, '').optional(),
  longitude: Joi.number().min(-180).max(180).allow(null, '').optional(),
  profile_image: Joi.any(),
...
module.exports = {
  ...
  registerShopOwnerSchema: Joi.object({
    ...
    shop_latitude: Joi.number().min(-90).max(90).allow(null, '').optional(),
    shop_longitude: Joi.number().min(-180).max(180).allow(null, '').optional(),
